...
void _linkedlist_qsort(linkedlist_t *l, int ( cmp_elements)(void *e1, void *e2), node_t *begin, node_t *end)
{
    ...
    while (lbound != rbound->right && lbound != rbound->right->right)
    {
        ...
        {
            // printf("> %s  -  %s\n", *(char **)(lbound->element), *(char **)(rbound->element));
    ...
    // printf("> %s  -  %s\n\n", *(char **)(rbound->element), *(char **)(pivot->element));
...
linkedlist_qsort (linkedlist_t * l, int ( cmp_elements)(void * e1, void * e2))
{
	/* TODO */
    node_t *b = l->right;
    _linkedlist_qsort(l, cmp_elements, b, l);
}